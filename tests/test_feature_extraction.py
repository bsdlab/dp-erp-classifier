...
def test_epoching(spawn_lsl_stream, spawn_lsl_marker_stream):
    ...
    epochs_accumulating_stack = []  # stack to collect currently accumulating epochs
    epochs_for_clf_stack = []
    while time.perf_counter() - start < 2:

        # get new data into the StreamWatchers and pass through the filter_bank
        ctx.update_stream_watchers()

        epochs_accumulating_stack.extend(get_new_epochs(ctx))
        ...
        while epochs_accumulating_stack != []:
            ...
            epo = epochs_accumulating_stack[0]
            ...
            if (epo.ts_event + epo.tmax) < curr_ts[-1]:
                ...
                epochs_for_clf_stack.append(epochs_accumulating_stack.pop(0))
        ...
        for epo in epochs_for_clf_stack:
            pass

        time.sleep(ctx.dt_s)

    # we should have 3 full epochs at this point
    assert len(epochs_for_clf_stack) == 3
    assert (
        np.abs(np.diff([e.ts_event for e in epochs_for_clf_stack]).mean() - 0.25) < 0.05
    )
    # marker stream for testing is created to have consecutive markers always with diff = +/-1
    assert (np.abs(np.diff([e.event for e in epochs_for_clf_stack])) == 1).all()
